...
A new **AI Analysis** page integrates a locally-running [Ollama](https://ollama.com/) instance (Llama3 model) to act as an AI Security Operations Center analyst.

### How it works

1. Admin selects filters (date range or single date, target device)
2. Admin selects a report type
3. The system fetches filtered security events + alerts from the database
4. Events are passed as context to the Ollama Llama3 model via its local API
5. The AI generates a structured security report and streams it to the dashboard
